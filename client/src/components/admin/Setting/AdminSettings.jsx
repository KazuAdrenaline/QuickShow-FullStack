import React, { useEffect, useState } from "react";
import { useUser } from "@clerk/clerk-react";
import Title from "../../admin/Title";
import BlurCircle from "../../BlurCircle";
import toast from "react-hot-toast";

const AdminSettings = () => {
  const { user, isLoaded } = useUser();

  const [firstName, setFirstName] = useState("");
  const [lastName, setLastName] = useState("");
  const [avatar, setAvatar] = useState("");
  const [avatarFile, setAvatarFile] = useState(null);

  useEffect(() => {
    if (isLoaded && user) {
      setFirstName(user.firstName || "");
      setLastName(user.lastName || "");
      setAvatar(user.imageUrl || "");
    }
  }, [isLoaded, user]);

  if (!isLoaded) return <p className="text-white p-6">Đang tải...</p>;

  /* -------------------------------------------------------------
      LƯU THAY ĐỔI HỒ SƠ
  ------------------------------------------------------------- */
  const saveProfile = async () => {
    try {
      let imageUrl = avatar;

      // Upload avatar nếu chọn ảnh mới
      if (avatarFile) {
        const uploaded = await user.setProfileImage({
          file: avatarFile,
        });

        imageUrl = uploaded.publicUrl;
      }

      // Update thông tin Clerk user
      await user.update({
        firstName,
        lastName,
        imageUrl,
      });

      toast.success("Cập nhật thông tin thành công!");
    } catch (err) {
      console.log(err);
      toast.error("Có lỗi xảy ra khi cập nhật!");
    }
  };

  return (
    <>
      <Title text1="Cài đặt" text2="Quản trị viên" />

      <div className="relative mt-10 max-w-3xl">
        <BlurCircle top="-50px" left="-50px" />

        {/* -------------------------------------------------------------
            THÔNG TIN HỒ SƠ
        ------------------------------------------------------------- */}
        <div className="bg-gray-900 border border-gray-700 p-6 rounded-xl shadow-lg">
          <h2 className="text-lg font-semibold text-white mb-4">
            Thông tin cá nhân
          </h2>

          {/* Avatar + upload */}
          <div className="flex items-center gap-6">
            <img
              src={avatar}
              className="w-20 h-20 rounded-full object-cover border border-gray-600"
              alt="avatar"
            />

            <div>
              <input
                type="file"
                accept="image/*"
                onChange={(e) => {
                  setAvatarFile(e.target.files[0]);
                  setAvatar(URL.createObjectURL(e.target.files[0]));
                }}
                id="avatar-upload"
                className="hidden"
              />

              <label
                htmlFor="avatar-upload"
                className="px-4 py-2 bg-primary text-black rounded cursor-pointer hover:bg-primary/80 transition"
              >
                Đổi ảnh đại diện
              </label>
            </div>
          </div>

          {/* Tên + họ */}
          <div className="mt-6 grid grid-cols-2 gap-4">
            <div>
              <label className="text-gray-400 text-sm">Tên</label>
              <input
                className="w-full mt-1 bg-gray-800 text-white p-2 rounded border border-gray-600 focus:ring-1 focus:ring-primary outline-none"
                value={firstName}
                onChange={(e) => setFirstName(e.target.value)}
              />
            </div>

            <div>
              <label className="text-gray-400 text-sm">Họ</label>
              <input
                className="w-full mt-1 bg-gray-800 text-white p-2 rounded border border-gray-600 focus:ring-1 focus:ring-primary outline-none"
                value={lastName}
                onChange={(e) => setLastName(e.target.value)}
              />
            </div>
          </div>

          {/* Email (không cho sửa) */}
          <div className="mt-4">
            <label className="text-gray-400 text-sm">Email</label>
            <input
              disabled
              className="w-full mt-1 bg-gray-800 text-gray-400 p-2 rounded border border-gray-600 cursor-not-allowed"
              value={user?.primaryEmailAddress?.emailAddress}
            />
          </div>

          {/* Button Lưu thay đổi */}
          <button
            onClick={saveProfile}
            className="mt-6 px-5 py-2 bg-primary text-black rounded hover:bg-primary/80 transition font-medium"
          >
            Lưu thay đổi
          </button>
        </div>
      </div>
    </>
  );
};

export default AdminSettings;
